...
class PricingRevision(ContractModel):
    revision_id: Identifier
    revision_number: Annotated[int, Field(ge=1)]
    created_at: datetime
    change_note: str = Field(default="", max_length=1000)
...
class WorkspacePricingAnalysis(ContractModel):
    analysis_id: Identifier
    name: str = Field(min_length=1, max_length=240)
    status: Literal["draft", "active", "archived"] = "active"
    created_at: datetime
    updated_at: datetime
    current_revision_id: Identifier
    revisions: list[PricingRevision] = Field(min_length=1)

    @model_validator(mode="after")
    def valid_revision_chain(self) -> WorkspacePricingAnalysis:
        revision_ids = [item.revision_id for item in self.revisions]
        if len(revision_ids) != len(set(revision_ids)):
            raise ValueError("pricing revision IDs must be unique")
        if [item.revision_number for item in self.revisions] != list(
            range(1, len(self.revisions) + 1)
        ):
            raise ValueError("pricing revision numbers must be contiguous and ordered")
        if self.current_revision_id != self.revisions[-1].revision_id:
            raise ValueError(
                "pricing current_revision_id must reference the latest revision"
            )
        return self

    @property
    def current_revision(self) -> PricingRevision:
        return self.revisions[-1]
...
class SustainableRevision(ContractModel):
    revision_id: Identifier
    revision_number: Annotated[int, Field(ge=1)]
    created_at: datetime
    change_note: str = Field(default="", max_length=1000)
...
class WorkspaceSustainableAnalysis(ContractModel):
    analysis_id: Identifier
    name: str = Field(min_length=1, max_length=240)
    status: Literal["draft", "active", "archived"] = "active"
    created_at: datetime
    updated_at: datetime
    current_revision_id: Identifier
    revisions: list[SustainableRevision] = Field(min_length=1)

    @model_validator(mode="after")
    def valid_revision_chain(self) -> WorkspaceSustainableAnalysis:
        revision_ids = [item.revision_id for item in self.revisions]
        if len(revision_ids) != len(set(revision_ids)):
            raise ValueError("sustainable-finance revision IDs must be unique")
        if [item.revision_number for item in self.revisions] != list(
            range(1, len(self.revisions) + 1)
        ):
            raise ValueError(
                "sustainable-finance revision numbers must be contiguous and ordered"
            )
        if self.current_revision_id != self.revisions[-1].revision_id:
            raise ValueError(
                "sustainable-finance current_revision_id must reference the latest revision"
            )
        return self

    @property
    def current_revision(self) -> SustainableRevision:
        return self.revisions[-1]


class GovernanceRevision(ContractModel):
    revision_id: Identifier
    revision_number: Annotated[int, Field(ge=1)]
    created_at: datetime
    change_note: str = Field(default="", max_length=1000)
...
class WorkspaceGovernanceAnalysis(ContractModel):
    analysis_id: Identifier
    name: str = Field(min_length=1, max_length=240)
    status: Literal["draft", "active", "archived"] = "active"
    created_at: datetime
    updated_at: datetime
    current_revision_id: Identifier
    revisions: list[GovernanceRevision] = Field(min_length=1)

    @model_validator(mode="after")
    def valid_revision_chain(self) -> WorkspaceGovernanceAnalysis:
        revision_ids = [item.revision_id for item in self.revisions]
        if len(revision_ids) != len(set(revision_ids)):
            raise ValueError("governance revision IDs must be unique")
        if [item.revision_number for item in self.revisions] != list(
            range(1, len(self.revisions) + 1)
        ):
            raise ValueError(
                "governance revision numbers must be contiguous and ordered"
            )
        if self.current_revision_id != self.revisions[-1].revision_id:
            raise ValueError(
                "governance current_revision_id must reference the latest revision"
            )
        return self

    @property
    def current_revision(self) -> GovernanceRevision:
        return self.revisions[-1]
...
class FinanceWorkspace(ContractModel):
    ...
    sustainable_analyses: list[WorkspaceSustainableAnalysis] = Field(
        default_factory=list
    )
    ...
    def unique_identifiers(self) -> FinanceWorkspace:
        ...
        if len(sustainable_ids) != len(set(sustainable_ids)):
            raise ValueError(
                "workspace sustainable-finance analysis IDs must be unique"
            )
