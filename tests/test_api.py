...
def test_model_registry_endpoint() -> None:
    ...
    assert all(model["model_version"] == "1.6.0" for model in models)
...
def test_evaluate_endpoint_uses_canonical_contract() -> None:
    ...
    assert result["contract_version"] == "1.6.0"
...
def test_uncertainty_endpoint_rejects_invalid_payload() -> None:
    response = CLIENT.post(
        ...
        json={"contract_version": "1.6.0", "model_id": "catalyst-finance.uncertainty"},
    )
    assert response.status_code == 422
